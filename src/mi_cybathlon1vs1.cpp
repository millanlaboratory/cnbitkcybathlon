...
int main(int argc, char *argv[]) {
	...
	//Tools for UDP communication to the game
	cybCommPeer2 =  new CcEndpoint();
	cybComm2 = new CcClient();
	cybCommPeer2->SetIp("192.168.1.3");
	cybCommPeer2->SetPortUInt(5555);

	while(cybComm2->Connect(cybCommPeer2->GetAddress(), CcSocket::UDP) == false) {
		CcLogInfo("Waiting for connection to UDP game command server...");
		CcTime::Sleep(1000);
	}
	CcLogInfo("Connection to UDP game command server established...");
			...
			if(probs->Max(&idx_hit) >= thresholds[idx_hit]) {
				...
				if(idx_hit == 0) {
					...
					for(int i=0;i<2;i++){
						...
						cybComm2->Send((const void*)(&comm),sizeof(int));
					}
				} else {
